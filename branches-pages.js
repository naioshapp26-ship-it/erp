(function () {
  ...
  const PAGE_CONFIGS = {
    'br-daily-operations': {
      ...
      subtitle: 'متابعة وتنفيذ العمليات اليومية للفرع',
      ...
      seed: [
        ['مزامنة بيانات الفرع', 'فريق التشغيل', 'قيد التنفيذ', '2026-06-09'],
        ['مراجعة طلبات الفرع', 'منسق الفرع', 'مكتمل', '2026-06-09'],
        ['تحديث لوحة المؤشرات', 'مدير الفرع', 'مجدول', '2026-06-09']
      ]
    },
    'br-sales': {
      title: 'المبيعات',
      subtitle: 'إدارة مبيعات الفرع والعروض والصفقات',
      icon: 'fa-chart-line',
    ...
    'br-subscriptions': {
      ...
      columns: ['الفرع', 'الخطة', 'الحالة', 'التجديد'],
      seed: [
        ['فرع نايوش الرئيسية', 'Enterprise', 'نشط', '2026-12-01'],
        ['فرع الرياض', 'Pro', 'نشط', '2026-09-15'],
        ['فرع جدة', 'Basic', 'تجريبي', '2026-07-01']
      ]
    },
    'br-training': {
      title: 'التدريب',
      subtitle: 'الدورات والبرامج التدريبية عبر الفرع',
    ...
    'br-customer-service': {
      title: 'خدمة العملاء',
      subtitle: 'تذاكر واستفسارات مستخدمي الفرع',
      icon: 'fa-headset',
    ...
    'br-operational-reports': {
      ...
      subtitle: 'مؤشرات أداء الفرع والتقارير الدورية',
      icon: 'fa-chart-pie',
      gradient: 'from-red-900 to-red-700',
      stats: [
    ...
    'br-local-hr': {
      ...
      subtitle: 'شؤون موظفي الفرع والحضور والطلبات',
      icon: 'fa-users',
      gradient: 'from-red-800 to-red-600',
      api: '/api/users',
      stats: [
    ...
    'br-operational-finance': {
      title: 'المالية التشغيلية',
      subtitle: 'المصروفات والتحصيل والميزانية التشغيلية للفرع',
      icon: 'fa-coins',
  ...
  function mapApiRows(route, payload) {
    ...
    if (route === 'br-customer-service' || route === 'br-daily-operations') {
      return list.slice(0, 12).map((item) => [
  ...
  function renderTable(route, config, rows) {
    const body = rows.length
      ? rows.map((row, index) => `
      <tr class="border-b border-slate-100 hover:bg-red-50/40 transition" data-br-row="${index}">
    ...
    return `
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden" data-br-table="${route}">
        <div class="px-5 py-4 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
          <h3 class="font-bold text-slate-800">سجل ${config.title}</h3>
          <div class="flex flex-wrap gap-2">
            <button type="button" data-br-action="refresh" data-route="${route}" class="px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold">
              <i class="fas fa-rotate"></i> تحديث
            </button>
            <button type="button" data-br-action="add" data-route="${route}" class="px-3 py-2 rounded-lg bg-red-700 hover:bg-red-800 text-white text-xs font-bold">
              <i class="fas fa-plus"></i> إضافة
            </button>
            <button type="button" data-br-action="export" data-route="${route}" class="px-3 py-2 rounded-lg border border-red-200 text-red-700 hover:bg-red-50 text-xs font-bold">
  ...
  function renderPage(route) {
    ...
    return `
      <div class="space-y-6" data-br-page="${route}">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ${config.stats.map((item) => `
            <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
              ...
              <p class="text-3xl font-black text-slate-800" data-br-stat="${route}:${item.key}">${stats[item.key] || 0}</p>
            </div>
          `).join('')}
        </div>
  ...
  function refreshStats(route, rows) {
    ...
    Object.keys(stats).forEach((key) => {
      const el = document.querySelector(`[data-br-stat="${route}:${key}"]`);
  ...
  function refreshPage(route, rows) {
    const config = PAGE_CONFIGS[route];
    if (!config) return;
    const page = document.querySelector(`[data-br-page="${route}"]`);
    if (!page) return;
    const table = page.querySelector(`[data-br-table="${route}"]`);
    if (table) {
  ...
  function closeModal() {
    document.getElementById('br-modal-overlay')?.remove();
  }
  ...
  function openRecordModal(route, mode, index) {
    ...
    const fields = config.columns.map((col, colIndex) => {
      ...
      return `
        <label class="block">
          ...
          <input type="text" name="br-field-${colIndex}" value="${escapeHtml(value)}" ${inputAttrs} />
        </label>
      `;
    }).join('');
    ...
    overlay.innerHTML = `
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" dir="rtl" role="dialog" aria-modal="true">
        ...
        <form id="br-record-form" class="p-6 space-y-4">
          ${fields}
          <div class="flex flex-wrap gap-2 pt-2 ${isView ? 'hidden' : ''}">
            ...
            <button type="button" data-br-modal-close class="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50">
              إلغاء
            </button>
          </div>
        </form>
    ...
    overlay.addEventListener('click', (event) => {
      if (event.target === overlay || event.target.closest('[data-br-modal-close]')) {
        closeModal();
      }
    });
    ...
    const form = overlay.querySelector('#br-record-form');
    form?.addEventListener('submit', (event) => {
      event.preventDefault();
      const values = config.columns.map((_, colIndex) => {
        const input = form.querySelector(`[name="br-field-${colIndex}"]`);
  ...
  async function handleBrAction(action, route, index) {
    const config = PAGE_CONFIGS[route];
    if (!config) return;

    if (action === 'refresh') {
      const button = document.querySelector(`[data-br-action="refresh"][data-route="${route}"]`);
  ...
  let brDelegationReady = false;

  function ensureBrDelegation() {
    if (brDelegationReady) return;
    brDelegationReady = true;
    document.addEventListener('click', (event) => {
      ...
      handleBrAction(action, route, index);
    });
  }

  window.BranchesPages = {
