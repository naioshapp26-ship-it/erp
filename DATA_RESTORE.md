...
### استعادة كتالوج الهيكل التنظيمي (فروع / حاضنات / منصات / مكاتب)

إذا اختفت الحاضنات والمنصات من صفحة **الهيكل الهرمي** بعد wipe أو bootstrap فقط:

1. الملف `hierarchy-catalog.json` فيه الكتالوج الأصلي المستخرج من `NaioshERP.sql` (27 فرع، 100 حاضنة، 95 منصة، قوالب مكاتب).
2. عند الإقلاع، `hierarchy-catalog-bootstrap.js` يعيد زرع الكتالوج تلقائياً إذا كان العدد أقل من العتبة.
3. إجبار الاستعادة يدوياً (مقر رئيسي فقط):
   ```bash
   curl -X POST https://YOUR-HOST/api/hierarchy/restore-catalog
   ```
   أو اضبط `RESTORE_HIERARCHY_CATALOG=true` ثم أعد النشر.
