...
async function testSalesAccountsOperationsAPI() {
    console.log('🧪 اختبار واجهات منظومة المبيعات والحسابات\n');
    ...
    for (const test of tests) {
        try {
            ...
            const dataArray = data.invoices || data.payments || data.customers || data.rows || data || [];
    ...
    if (passedTests === tests.length) {
        console.log('🎉 جميع اختبارات منظومة المبيعات والحسابات نجحت!\n');
        process.exit(0);
    } else {
...
testSalesAccountsOperationsAPI();
