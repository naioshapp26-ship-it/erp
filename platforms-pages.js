(function () {
  ...
  const STORAGE_PREFIX = 'platforms:data:';

  const PAGE_CONFIGS = {
    'pl-daily-operations': {
      title: 'العمليات اليومية',
      subtitle: 'متابعة وتنفيذ العمليات اليومية للمنصة',
      ...
      seed: [
        ['مزامنة بيانات المنصة', 'فريق التشغيل', 'قيد التنفيذ', '2026-06-09'],
        ['مراجعة طلبات المنصة', 'منسق المنصة', 'مكتمل', '2026-06-09'],
        ['تحديث لوحة المؤشرات', 'مدير المنصة', 'مجدول', '2026-06-09']
      ]
    },
    'pl-sales': {
      title: 'المبيعات',
      subtitle: 'إدارة مبيعات المنصة والعروض والصفقات',
      icon: 'fa-chart-line',
    ...
    'pl-subscriptions': {
      ...
      columns: ['المنصة', 'الخطة', 'الحالة', 'التجديد'],
      seed: [
        ['منصة نايوش الرئيسية', 'Enterprise', 'نشط', '2026-12-01'],
        ['منصة الرياض', 'Pro', 'نشط', '2026-09-15'],
        ['منصة جدة', 'Basic', 'تجريبي', '2026-07-01']
      ]
    },
    'pl-training': {
      title: 'التدريب',
      subtitle: 'الدورات والبرامج التدريبية عبر المنصة',
    ...
    'pl-customer-service': {
      title: 'خدمة العملاء',
      subtitle: 'تذاكر واستفسارات مستخدمي المنصة',
      icon: 'fa-headset',
    ...
    'pl-operational-reports': {
      title: 'التقارير التشغيلية',
      subtitle: 'مؤشرات أداء المنصة والتقارير الدورية',
      icon: 'fa-chart-pie',
      gradient: 'from-red-900 to-red-700',
      stats: [
    ...
    'pl-local-hr': {
      ...
      subtitle: 'شؤون موظفي المنصة والحضور والطلبات',
      icon: 'fa-users',
      gradient: 'from-red-800 to-red-600',
      api: '/api/users',
      stats: [
    ...
    'pl-operational-finance': {
      title: 'المالية التشغيلية',
      subtitle: 'المصروفات والتحصيل والميزانية التشغيلية للمنصة',
      icon: 'fa-coins',
  ...
  function mapApiRows(route, payload) {
    ...
    if (route === 'pl-sales' || route === 'pl-operational-finance') {
      return list.slice(0, 12).map((item) => [
    ...
    if (route === 'pl-customer-service' || route === 'pl-daily-operations') {
      return list.slice(0, 12).map((item) => [
  ...
  function renderRowActions(route, index) {
    return `
      <div class="flex flex-wrap items-center justify-end gap-1.5">
        <button type="button" data-pl-action="view" data-route="${route}" data-index="${index}"
          class="px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold" title="عرض">
          <i class="fas fa-eye"></i>
        </button>
        <button type="button" data-pl-action="edit" data-route="${route}" data-index="${index}"
          class="px-2.5 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold" title="تعديل">
          <i class="fas fa-pen"></i>
        </button>
        <button type="button" data-pl-action="delete" data-route="${route}" data-index="${index}"
  ...
  function renderTable(route, config, rows) {
    const body = rows.length
      ? rows.map((row, index) => `
      <tr class="border-b border-slate-100 hover:bg-red-50/40 transition" data-pl-row="${index}">
    ...
    return `
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden" data-pl-table="${route}">
        <div class="px-5 py-4 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
          <h3 class="font-bold text-slate-800">سجل ${config.title}</h3>
          <div class="flex flex-wrap gap-2">
            <button type="button" data-pl-action="refresh" data-route="${route}" class="px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold">
              <i class="fas fa-rotate"></i> تحديث
            </button>
            <button type="button" data-pl-action="add" data-route="${route}" class="px-3 py-2 rounded-lg bg-red-700 hover:bg-red-800 text-white text-xs font-bold">
              <i class="fas fa-plus"></i> إضافة
            </button>
            <button type="button" data-pl-action="export" data-route="${route}" class="px-3 py-2 rounded-lg border border-red-200 text-red-700 hover:bg-red-50 text-xs font-bold">
  ...
  function renderPage(route) {
    ...
    return `
      <div class="space-y-6" data-pl-page="${route}">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ${config.stats.map((item) => `
            <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
              ...
              <p class="text-3xl font-black text-slate-800" data-pl-stat="${route}:${item.key}">${stats[item.key] || 0}</p>
            </div>
          `).join('')}
        </div>
  ...
  async function fetchRows(route) {
    ...
    try {
      ...
      console.warn('[Platforms] API fallback for', route, error.message);
    }
  ...
  function refreshStats(route, rows) {
    ...
    Object.keys(stats).forEach((key) => {
      const el = document.querySelector(`[data-pl-stat="${route}:${key}"]`);
  ...
  function refreshPage(route, rows) {
    const config = PAGE_CONFIGS[route];
    if (!config) return;
    const page = document.querySelector(`[data-pl-page="${route}"]`);
    if (!page) return;
    const table = page.querySelector(`[data-pl-table="${route}"]`);
    if (table) {
  ...
  function closeModal() {
    document.getElementById('pl-modal-overlay')?.remove();
  }
  ...
  function openRecordModal(route, mode, index) {
    ...
    const fields = config.columns.map((col, colIndex) => {
      ...
      return `
        <label class="block">
          ...
          <input type="text" name="pl-field-${colIndex}" value="${escapeHtml(value)}" ${inputAttrs} />
        </label>
      `;
    }).join('');
    ...
    overlay.innerHTML = `
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" dir="rtl" role="dialog" aria-modal="true">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <button type="button" data-pl-modal-close class="w-9 h-9 rounded-lg hover:bg-slate-100 text-slate-500">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <form id="pl-record-form" class="p-6 space-y-4">
          ${fields}
          <div class="flex flex-wrap gap-2 pt-2 ${isView ? 'hidden' : ''}">
            ...
            <button type="button" data-pl-modal-close class="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50">
              إلغاء
            </button>
          </div>
        </form>
    ...
    overlay.addEventListener('click', (event) => {
      if (event.target === overlay || event.target.closest('[data-pl-modal-close]')) {
        closeModal();
      }
    });
    ...
    const form = overlay.querySelector('#pl-record-form');
    form?.addEventListener('submit', (event) => {
      event.preventDefault();
      const values = config.columns.map((_, colIndex) => {
        const input = form.querySelector(`[name="pl-field-${colIndex}"]`);
  ...
  async function handlePlAction(action, route, index) {
    const config = PAGE_CONFIGS[route];
    if (!config) return;

    if (action === 'refresh') {
      const button = document.querySelector(`[data-pl-action="refresh"][data-route="${route}"]`);
  ...
  function ensurePlDelegation() {
    ...
    document.addEventListener('click', (event) => {
      const button = event.target.closest('[data-pl-action]');
      if (!button) return;
      event.preventDefault();
      ...
      handlePlAction(action, route, index);
    });
  }

  window.PlatformsPages = {
