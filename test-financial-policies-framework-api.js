/**
 * 🧪 Test Financial Policies Framework API Endpoints (Page 39)
 */
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log(`📊 Data Summary:`, {
                ...
                rows: data.invoices?.length ?? data.payments?.length ?? data.expenses?.length ?? data.assets?.length ?? data.plans?.length ?? data.entries?.length ?? data.rows?.length ?? 0
            });
...
async function runAllTests() {
    ...
    console.log('🧪 اختبار Backend API - السياسات المالية الأساسية (صفحة 39)');
    console.log('='.repeat(80));
    ...
    results.total++;
    const t3 = await testAPI('3️⃣ المصروفات', `${BASE_URL}/finance/expenses?entity_id=${ENTITY_ID}`);
    if (t3.success) results.passed++; else results.failed++;

    results.total++;
    const t4 = await testAPI('4️⃣ الأصول الثابتة', `${BASE_URL}/finance/fixed-assets?entity_id=${ENTITY_ID}`);
    if (t4.success) results.passed++; else results.failed++;

    results.total++;
    const t5 = await testAPI('5️⃣ خطط السداد', `${BASE_URL}/finance/payment-plans?entity_id=${ENTITY_ID}`);
