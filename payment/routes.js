...
router.post('/credits/checkout', async (req, res) => {
  ...
  try {
    ...
    const session = await tenantService.createCheckoutSession(req.tenantPool, {
      ...
      successUrl: `${baseUrl}/dashboard.html#credit-topup?status=success`,
      cancelUrl: `${baseUrl}/dashboard.html#credit-topup?status=cancelled`,
      returnUrl: `${baseUrl}/dashboard.html#credit-topup?status=success`,
      customerEmail,
      metadata,
